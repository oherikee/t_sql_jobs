...
-- The "tb_third_trend_period" table is the third of three tables and will be consumed by the system for a separate 
...
CREATE TRIGGER trg_third_trend_period
...
BEGIN
	...
	DELETE FROM tb_third_trend_period WHERE @id = tb_third_trend_period.id_civil_work;
	-- 04C
	...
	SELECT
		...
		CASE
			WHEN tde_accumulated.period> sq_sd.status_date
				THEN NULL
	...
	FROM
		...
		FROM
			...
			JOIN
				(SELECT DISTINCT
					tb_civil_work.id_civil_work,
					sq_previous_calendar_date.calendar_period previous_date,
					tb_calendar.calendar_period current_date
				FROM
					tb_civil_work
				JOIN
					tb_term
					ON tb_term.id_civil_work = tb_civil_work.id_civil_work
				JOIN
					tb_term_input_data
					ON tb_term_input_data.id_term = tb_term.id_term
				JOIN
					tb_calendar
					ON tb_calendar.id_input_data = tb_term_input_data.id_input_data
				LEFT JOIN
					tb_calendar sq_previous_calendar_date
					ON sq_previous_calendar_date.SDC_COD = tb_calendar.SDC_COD - 1) dynamic_calendar
				ON dynamic_calendar.id_civil_work = tb_civil_work.id_civil_work
			JOIN tb_schedule ON tb_civil_work.id_civil_work = tb_schedule.id_civil_work
			...
			JOIN tb_activty_foreseen_progress ON tb_qualitative_activity.id_qualitative_activity = tb_activty_foreseen_progress.id_qualitative_activity AND ((CAST(tb_activty_foreseen_progress.id_term_date AS DATE) > dynamic_calendar.previous_date AND CAST(tb_activty_foreseen_progress.id_term_date AS DATE) <= dynamic_calendar.current_date) OR (dynamic_calendar.previous_date IS NULL AND CAST(tb_activty_foreseen_progress.id_term_date AS DATE) <= dynamic_calendar.current_date))
			LEFT JOIN tb_activity_real_progress ON tb_qualitative_activity.id_qualitative_activity = tb_activity_real_progress.id_qualitative_activity AND ((CAST(tb_activity_real_progress.date AS DATE) > dynamic_calendar.previous_date AND CAST(tb_activity_real_progress.date AS DATE) <= dynamic_calendar.current_date) OR (dynamic_calendar.previous_date IS NULL AND CAST(tb_activity_real_progress.date AS DATE) <= dynamic_calendar.current_date))        
		GROUP BY
			tb_civil_work.id_civil_work,
	...
	-- The snippet below is responsible for fetching the status date (point where the project stands);
		(SELECT 
			sq_cw.id_civil_work,
			MAX(avr.date) AS sq_status_date
		FROM
			tb_civil_work sq_cw
		LEFT JOIN
			tb_term sq_term
		...
		LEFT JOIN
			...
			ON mpa.id_term = sq_term.id_term
		LEFT JOIN
	...
	INSERT INTO tb_third_trend_period
			   (period
			   ,initial_deviation
			   ,mobile_deviation
			   ,previous_estimate
	...
	JOIN
		...
		-- snippet below, everything is structured with the necessary information to assemble the third period of the 
		...
		FROM
			(SELECT
				...
				tde_accumulated.foreseen_progress,
				CASE
					WHEN tb_second_trend_period.initial_deviation IS NOT NULL
					THEN tb_second_trend_period.initial_deviation
					ELSE tb_second_trend_period.trend_period / tde_accumulated.foreseen_progress
				END AS initial_deviation,
				tb_second_trend_period.trend_period
			FROM
				tde_accumulated
			JOIN
				tb_second_trend_period
				ON tb_second_trend_period.id_civil_work = tde_accumulated.id_civil_work
				AND tb_second_trend_period.period = CAST(tde_accumulated.period AS DATE)) sq_initial_deviation
		LEFT JOIN
			(SELECT TOP 3
				tde_accumulated.id_civil_work,
				tde_accumulated.period,
			...
			JOIN
				tb_second_trend_period
				ON tb_second_trend_period.id_civil_work = tde_accumulated.id_civil_work
				AND tb_second_trend_period.period = CAST(tde_accumulated.period AS DATE)
			WHERE
				tb_second_trend_period.trend_period / tde_accumulated.foreseen_progress IS NOT NULL
			ORDER BY
...
ALTER TABLE [dbo].[tb_activity_real_progress] ENABLE TRIGGER [trg_third_trend_period];
GO